...
pub fn handle(ctx_ptr: ?*anyopaque, allocator: std.mem.Allocator, req: *const Request, res: *Response) !void {
    ...
    web.log.logFmt(.info, "stripe webhook: body={d}b sig={d}b", .{
        req.body.len,
        if (req.getHeader("Stripe-Signature")) |h| h.len else 0,
    });
    ...
    if (!verifyStripeSignature(allocator, ctx.stripe_webhook_secret, sig_header, req.body)) {
        web.log.logFmt(.warn, "stripe webhook: signature verify failed", .{});
    ...
    web.log.logFmt(.info, "stripe webhook: type={s} intent={s}", .{ event_type, intent_id });
    ...
    if (new_status) |status| {
        try cross_service.advanceOrderStatus(ctx, allocator, req.io.?, intent_id, status);
    }
