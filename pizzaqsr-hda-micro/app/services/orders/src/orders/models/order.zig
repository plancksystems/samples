...
pub const Order = struct {
    ...
    pub fn isStatusReached(self: Order, target: []const u8) bool {
        ...
        var found_tgt = false;
        for (stages, 0..) |s, i| {
            if (std.mem.eql(u8, s, self.Status)) {
                cur_idx = i;
                found_cur = true;
            }
    ...
    pub fn isCancelled(self: Order) bool {
        return std.mem.eql(u8, self.Status, status_cancelled);
    }

    pub fn isStatusCurrent(self: Order, target: []const u8) bool {
        return std.mem.eql(u8, self.Status, target);
    }

    pub fn statusBadge(self: Order) []const u8 {
        if (std.mem.eql(u8, self.Status, status_awaiting_payment)) return "⏳";
        if (std.mem.eql(u8, self.Status, status_paid)) return "✅";
        if (std.mem.eql(u8, self.Status, status_preparing)) return "🍳";
        if (std.mem.eql(u8, self.Status, status_ready)) return "📦";
        if (std.mem.eql(u8, self.Status, status_out_for_delivery)) return "🛵";
        if (std.mem.eql(u8, self.Status, status_delivered)) return "🎉";
        if (std.mem.eql(u8, self.Status, status_cancelled)) return "❌";
        return "";
    }
    ...
    pub fn statusMessage(self: Order) []const u8 {
        if (std.mem.eql(u8, self.Status, status_awaiting_payment)) return "Waiting for payment to clear…";
        if (std.mem.eql(u8, self.Status, status_paid)) return "Payment confirmed. Off to the kitchen!";
        if (std.mem.eql(u8, self.Status, status_preparing)) return "Your order is being prepared.";
        if (std.mem.eql(u8, self.Status, status_ready)) return "Ready for pickup by delivery.";
        if (std.mem.eql(u8, self.Status, status_out_for_delivery)) return "On the way to you!";
        if (std.mem.eql(u8, self.Status, status_delivered)) return "Delivered. Enjoy your meal!";
        if (std.mem.eql(u8, self.Status, status_cancelled)) return "Order cancelled.";
        return "";
    }
};
