...
pub fn build(b: *std.Build) void {
    ...
    const cart_clean = b.addSystemCommand(&.{ "planctl", "clean", "src/cart/fragments/" });
    const cart_preprocess = b.addSystemCommand(&.{ "planctl", "src/cart/zsx/", "src/cart/fragments/" });
    cart_preprocess.step.dependOn(&cart_clean.step);

    const checkout_clean = b.addSystemCommand(&.{ "planctl", "clean", "src/checkout/fragments/" });
    const checkout_preprocess = b.addSystemCommand(&.{ "planctl", "src/checkout/zsx/", "src/checkout/fragments/" });
    checkout_preprocess.step.dependOn(&checkout_clean.step);

    const orders_clean = b.addSystemCommand(&.{ "planctl", "clean", "src/orders/fragments/" });
    const orders_preprocess = b.addSystemCommand(&.{ "planctl", "src/orders/zsx/", "src/orders/fragments/" });
