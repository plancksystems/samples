...
pub fn handle(ctx_ptr: ?*anyopaque, handler_allocator: Allocator, req: *const schnell.Request, writer: *std.Io.Writer) anyerror!void {
    ...
    if (!ctx.bus.canAcceptSubscriber()) {
        log.warn("kitchen: subscriber cap reached, rejecting", .{});
        return error.ServiceUnavailable;
    }

    const last_event_id = parseLastEventId(req.getHeader("Last-Event-ID"));

    log.info("kitchen: subscribe last_event_id={d}", .{last_event_id});
    defer log.info("kitchen: exit", .{});
    ...
    try ctx.bus.subscribe("kitchen", sub, .{ .last_event_id = last_event_id });
    defer ctx.bus.unsubscribe("kitchen", sub);
